...
//    Gup->computeGreenNaive(Bup.list(), L - 1); Gdown->computeGreenNaive(Bdown.list(), L - 1);
    ...
    for (step = 0; step < totalMCSteps; step++)
    {
        ...
        else
        {
            ...
            electronDensities[sweep] += ( electronDensity * sign - electronDensities[sweep] ) / ( l + 1 ) ;
            doubleOcs[sweep] +=  ( doubleOc * sign - doubleOcs[sweep] ) / ( l + 1 ) ;
            magCorrs[sweep] += (magCorr * sign - magCorrs[sweep] ) / ( l + 1 );
            signs[sweep] = sign;
...
//                    //  Using the BlockOfGreens Method, we can obtain time-displaced Green's as well
    ...
    //  SAVE OUTPUT.
    std::ofstream file0("plots/simulationParameters.txt");
    if (file0.is_open())
    {
        file0 << "N\t" << NSITES << '\n'; file0 << "dt\t" << dt << '\n';
        file0 << "BETA\t" << BETA << '\n'; file0 << "L\t" << L << '\n';
        file0 << "t\t" << HOPPING << '\n'; file0 << "U\t" << U << '\n';
        file0 << "mu\t" << mu << '\n'; file0 << "totalMCSweeps\t" << totalMCSweeps << '\n';
        file0 << "GREEN_AFRESH_FREQ\t" << GREEN_AFRESH_FREQ << '\n'; file0 << "Lbda\t" << Lbda << '\n';
    } file0.close();
    //  STORE MEASUREMENTS
    std::ofstream file1("plots/LOGweights.txt");
    std::ofstream file2("plots/signs.txt");
