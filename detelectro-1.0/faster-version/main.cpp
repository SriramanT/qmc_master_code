...
{
    if ( argc != 5) //  U, mu, #sweeps
    {
        return -1;
    }
    //  SET SIMULATION PARAMETERS.
    const int N = 20;  //  # sites
    ...
    const int greenAfreshFreq = 3;  //   how often to calculate Green's functions afresh (in # im-time slices)
    ...
    double dt = 1. / dtInv;  //  Trotter error, or time subinterval width. error scales as dt^2
    ...
    double decisionMaker; int totalMCSteps = totalMCSweeps * N * L;
...
//    K.oneDimensionalChainOBC(t, dt, mu);
    //  SQUARE LATTICE PBC
//    K.twoDimensionalRectanglePBC(sqrt(N), t, dt, mu);
    //  SQUARE LATTICE OBC
//    K.twoDimensionalRectangleOBC(N / 2, t, dt, mu);
...
//    K.twoDimensionalRectangleOBC(N / 2, t, dt, mu);
    ...
    //  NANORIBBON
//    int Ny = 2;    K.nanoribbon(Ny, t, dt, mu);
    ...
    Green< N, L, Lbda> * Gup = new Green< N, L, Lbda>; Green< N, L, Lbda> * Gdown = new Green< N, L, Lbda>;
...
//    Gup->computeGreenNaive(Bup.list(), L - 1); Gdown->computeGreenNaive(Bdown.list(), L - 1);
    ...
    int * sign = new int[totalMCSweeps * L];
//    double * electronDensities = new double[totalMCSweeps];
//    double * doubleOcs = new double[totalMCSweeps];
//    Eigen::Matrix<double, N, N> * magCorrs = new Eigen::Matrix<double, N, N>[totalMCSweeps];
//    Eigen::Matrix<double, N, N> * uneqMagCorrs = new Eigen::Matrix<double, N, N>[totalMCSweeps];
...
//    uneqMagCorrs[0] = Eigen::Matrix<double, N, N>::Zero();
    ...
    double doubleOc;
    double meanDoubleOc = 0.;
    Eigen::Matrix<double, N, N> magCorr;
    Eigen::Matrix<double, N, N> meanMagCorr = Eigen::Matrix<double, N, N>::Zero();
//    electronDensities[0] = 0.; doubleOcs[0] = 0.;
    ...
    for (step = 0; step < totalMCSteps; step++)
    {
        ...
        else
        {
            ...
            weights[sweep * L + l] = weight; sign[sweep * L + l] = std::copysign(1., dUp * dDown);
...
//                    }
                ...
                meanDensity += ( electronDensity - meanDensity ) / ( (sweep - W) * L + l + 1 );
                meanDoubleOc += ( doubleOc - meanDoubleOc ) / ( (sweep - W) * L + l + 1 );
                meanMagCorr += (magCorr - meanMagCorr ) / ( (sweep - W) * L + l + 1 );
            }
...
//                    uneqMagCorrs[sweep] = Eigen::Matrix<double, N, N>::Zero();
    ...
    std::cout << "Average Sign: " << meanSign << std::endl << std::endl;
    std::cout << "Average Density: " << meanDensity << std::endl << std::endl;
    std::cout << "Average Double Oc: " << meanDoubleOc << std::endl << std::endl;
   std::cout << "Average Mag Corr: " << meanMagCorr << std::endl << std::endl;
...
//        file5 << std::left;
        for (int s = 0; s < totalMCSweeps; s++)
        {
            for (int slice = 0; slice < L; slice++)
            {
                ...
                file2 << std::left << std::setw(25) << sign[s * L + slice] << '\n';
            }
...
//            file5 << std::setprecision(10) << uneqMagCorrs[s] << std::endl << std::endl;
        ...
        file4 << std::setprecision(10) << magCorr << std::endl << std::endl;
        file2 << '\n';
//        file3 << '\n'; file4 << '\n'; file5 << '\n';
    }
    file2.close();
...
//    file5.close();
    ...
    delete[] weights;
//    delete[] doubleOcs; delete[] electronDensities; delete[] magCorrs; delete[] uneqMagCorrs;
